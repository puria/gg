...
func TestShellInitPassesThroughManagementAliases(t *testing.T) {
	...
	if !strings.Contains(fishScript, "list ls status starship prune rm") {
	...
	if !strings.Contains(bashScript, "|list|ls|status|starship|prune|rm)") {
...
func TestRunHelpVariants(t *testing.T) {
	...
	for _, arg := range cases {
		t.Run(arg, func(t *testing.T) {
			...
			for _, want := range []string{"alias", "list", "status", "starship", "prune", "shell-init"} {
