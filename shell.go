...
const fishInitTemplate = `function gg --description 'manage git repos'
    ...
    switch "$argv[1]"
    case help -h --help version --version shell-init config-path init-config path alias list status prune
        $gg_bin $argv
        return $status
    end
...
const posixInitTemplate = `gg() {
  ...
  case "$1" in
    ""|help|-h|--help|version|--version|shell-init|config-path|init-config|path|alias|list|status|prune)
      "$gg_bin" "$@"
      return $?
      ;;
  esac
